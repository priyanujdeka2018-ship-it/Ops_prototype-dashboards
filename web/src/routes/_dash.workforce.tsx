// Module C · Workforce Quality — 3-level progressive disclosure, reading the
// Python-emitted data.workforce (no client derivation).
// L1: region · L2 (?workType=X): work-type teams · L3 (?teamId=Y): team detail
import { createFileRoute, Link, useNavigate } from "@tanstack/react-router";
import { useDash } from "@/dashboards/app-context";
// @ts-expect-error jsx
import { Breadcrumb, ThreadNav, PillRow, Panel, AurRiskBadge, RiskMeter, DriverList, CoachingCard, aurMono, aurSerif } from "@/dashboards/atoms.jsx";
// @ts-expect-error jsx
import { WORK_TYPE_LABELS, classifyMetric, fmt } from "@/dashboards/data-utils.jsx";
...
function Workforce() {
  ...
  const { workType, teamId } = Route.useSearch() as any;
  if (!data) return <Loading AUR={AUR} />;
  const q = data.workforce;
  if (teamId) return <TeamLevel q={q} teamId={teamId} AUR={AUR} />;
  if (workType) return <WorkTypeLevel data={data} q={q} workType={workType} AUR={AUR} />;
...
function Overview({ q, AUR }: any) {
  ...
  const queue = q.teams.filter((t: any) => t.risk_band !== "Insufficient").slice(0, 6);
        ...
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {q.byWorkType.map((w: any) => (
              <Link key={w.work_type} to="/workforce" search={((prev: any) => ({ ...prev, workType: w.work_type, teamId: undefined, contributorId: undefined })) as any} style={{ textDecoration: "none", color: "inherit" }}>
                <Panel AUR={AUR} hoverable pad={18}>
                  <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr auto", gap: 16, alignItems: "center" }}>
                    <div>
                      ...
                      <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, marginTop: 3 }}>quality {w.avgQuality} · {w.teamsAtRisk} teams at risk · {w.flagged} flagged</div>
                    </div>
                    <div>
        ...
        <div>
          ...
          <Panel AUR={AUR} pad={0} style={{ overflow: "hidden" }}>
            {queue.map((t: any, i: number) => (
              <Link key={t.team_id} to="/workforce" search={((prev: any) => ({ ...prev, workType: t.work_type, teamId: t.team_id })) as any}
                style={{
                  display: "grid", gridTemplateColumns: "1fr auto auto", gap: 14, alignItems: "center",
                  padding: "16px 20px", borderBottom: i === queue.length - 1 ? "none" : `1px solid ${AUR.border}`,
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: AUR.textFaint, marginTop: 3 }}>{t.name}{t.drivers[0] ? ` · ${t.drivers[0].toLowerCase()}` : ""}</div>
                </div>
                <span style={{ fontFamily: aurSerif, fontSize: 20, color: t.risk_band === "High" ? AUR.bad : t.risk_band === "Medium" ? AUR.warn : AUR.text }}>{t.risk_score}</span>
...
// ─── L2 · Work-type ─────────────────────────────────────────────────────────
function WorkTypeLevel({ data, q, workType, AUR }: any) {
  const { densityPreset } = useDash();
  const navigate = useNavigate();
  const teams = q.teams.filter((t: any) => t.work_type === workType);
  const flagged = q.contributors.filter((c: any) => c.work_type === workType).slice(0, 6);

  return (
    <>
      <Breadcrumb AUR={AUR} items={[
        { label: "Module C · Workforce Quality", to: "/workforce", search: ((prev: any) => ({ ...prev, workType: undefined, teamId: undefined, contributorId: undefined })) },
        { label: WORK_TYPE_LABELS[workType] },
      ]} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.accent, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 10 }}>Module C · {WORK_TYPE_LABELS[workType]}</div>
        ...
        <PillRow AUR={AUR} options={data.workTypeRollup.map((r: any) => r.work_type)} value={workType}
          onChange={(v: string) => navigate({ to: "/workforce", search: ((prev: any) => ({ ...prev, workType: v, teamId: undefined })) as any })}
      ...
      <Panel AUR={AUR} pad={0} style={{ overflow: "hidden", marginTop: densityPreset.sectionGap }}>
        ...
        {teams.map((t: any, i: number) => (
          <Link key={t.team_id} to="/workforce" search={((prev: any) => ({ ...prev, workType, teamId: t.team_id })) as any}
            style={{
            ...
            <div>
              <span style={{ fontFamily: aurMono, fontSize: 12, color: AUR.text }}>{t.team_id.replace("TEAM_APAC_", "")}</span>
              <div style={{ fontSize: 11.5, color: AUR.textFaint, marginTop: 2 }}>{t.name} · {t.contributors} heads</div>
            </div>
            ...
            <span style={{ fontFamily: aurMono, fontSize: 12.5, color: t.drift < 0 ? AUR.bad : AUR.good, textAlign: "right" }}>{t.drift > 0 ? "+" : ""}{t.drift}</span>
            <span style={{ fontFamily: aurMono, fontSize: 12.5, color: t.goldFail > 18 ? AUR.bad : t.goldFail > 10 ? AUR.warn : AUR.text, textAlign: "right" }}>{t.goldFail}%</span>
            <span style={{ fontFamily: aurMono, fontSize: 12.5, color: t.rework > 10 ? AUR.warn : AUR.text, textAlign: "right" }}>{t.rework}%</span>
            <span style={{ textAlign: "right" }}><AurRiskBadge AUR={AUR} level={t.risk_band} /></span>
            <span style={{ fontFamily: aurSerif, fontSize: 20, color: t.risk_band === "High" ? AUR.bad : t.risk_band === "Medium" ? AUR.warn : AUR.text, textAlign: "right" }}>{t.risk_score}</span>
      ...
      {flagged.length > 0 && (
        <>
          <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, letterSpacing: 0.6, textTransform: "uppercase", marginTop: densityPreset.sectionGap, marginBottom: 16 }}>Contributors flagged for support · {WORK_TYPE_LABELS[workType]}</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 12 }}>
            {flagged.map((c: any) => <CoachingCard key={c.contributor_id} c={c} AUR={AUR} />)}
          </div>
        </>
      )}

      <ThreadNav AUR={AUR} density={densityPreset} items={[
        { kicker: "Module A", title: "See the health board", hint: `SLA, CSAT and escalation pressure for ${WORK_TYPE_LABELS[workType]}.`, to: "/health", search: ((prev: any) => ({ ...prev, workType, teamId: undefined })) },
        { kicker: "Module D", title: "Is capacity straining quality", hint: "When heads are stretched, quality slips first.", to: "/capacity", search: ((prev: any) => ({ ...prev, workType, teamId: undefined })) },
      ]} />
    </>
  );
}
...
// ─── L3 · Team ──────────────────────────────────────────────────────────────
function TeamLevel({ q, teamId, AUR }: any) {
  const { densityPreset } = useDash();
  const t = q.teams.find((x: any) => x.team_id === teamId);
  if (!t) return <div style={{ padding: 40, color: AUR.textFaint, textAlign: "center", fontStyle: "italic" }}>Team not found.</div>;
  const flagged = q.contributors.filter((c: any) => c.team_id === t.team_id);
  const subs = [
    { label: "Quality",   value: t.quality, status: classifyMetric("quality", t.quality) },
    { label: "Drift",     value: `${t.drift > 0 ? "+" : ""}${t.drift}`, status: t.drift < 0 ? "bad" : "good" },
    { label: "Gold-fail", value: `${t.goldFail}%`, status: t.goldFail > 18 ? "bad" : t.goldFail > 10 ? "warn" : "good" },
    { label: "Override",  value: `${t.override}%`, status: t.override > 16 ? "bad" : t.override > 8 ? "warn" : "good" },
    { label: "Peer agree",value: `${t.peerAgree}%`, status: t.peerAgree < 75 ? "bad" : t.peerAgree < 85 ? "warn" : "good" },
    { label: "Rework",    value: `${t.rework}%`, status: t.rework > 10 ? "warn" : "good" },
  ];
  ...
  return (
    <>
      <Breadcrumb AUR={AUR} items={[
        { label: "Module C · Workforce Quality", to: "/workforce", search: ((prev: any) => ({ ...prev, workType: undefined, teamId: undefined, contributorId: undefined })) },
        { label: WORK_TYPE_LABELS[t.work_type], to: "/workforce", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: undefined })) },
        { label: t.team_id.replace("TEAM_APAC_", "") },
      ]} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.accent, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 10 }}>{WORK_TYPE_LABELS[t.work_type]} · {t.contributors} contributors · {t.lowTenure}% low-tenure</div>
          <h1 style={{ fontFamily: aurSerif, fontSize: 36, fontWeight: 400, letterSpacing: -1, margin: 0, lineHeight: 1.1, color: AUR.text }}>{t.name}'s team · quality risk</h1>
          <p style={{ color: AUR.textDim, fontSize: 14, marginTop: 12, maxWidth: 720, lineHeight: 1.6 }}>
            Quality {t.quality}, {t.drift < 0 ? `down ${Math.abs(t.drift)}pt` : `up ${t.drift}pt`} since last period. The drivers below are exactly what the deterministic scorer flagged.
          </p>
        </div>
        <AurRiskBadge AUR={AUR} level={t.risk_band} />
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Panel AUR={AUR}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 18 }}>
              <span style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, letterSpacing: 0.6, textTransform: "uppercase" }}>Risk drivers</span>
              <span style={{ fontFamily: aurSerif, fontSize: 34, color: t.risk_band === "High" ? AUR.bad : t.risk_band === "Medium" ? AUR.warn : AUR.text }}>{t.risk_score}</span>
            </div>
            <DriverList AUR={AUR} drivers={t.drivers} max={6} />
          </Panel>
          <Panel AUR={AUR} glow accentEdge>
            <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.accent, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 10 }}>Recommended action</div>
            <div style={{ fontFamily: aurSerif, fontSize: 22, color: AUR.text, letterSpacing: -0.4, lineHeight: 1.3 }}>{t.action}.</div>
          </Panel>
        </div>
      ...
      <ThreadNav AUR={AUR} density={densityPreset} items={[
        { kicker: "Module A", title: "This team's health record", hint: "Escalations, root causes and SLA.", to: "/health", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: t.team_id })) },
        { kicker: "Module D", title: "This team's capacity load", hint: "Is the team simply over capacity?", to: "/capacity", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: t.team_id })) },
        { kicker: "Module C", title: `Back to ${WORK_TYPE_LABELS[t.work_type]}`, hint: "Return to the work-type comparison.", to: "/workforce", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: undefined })) },
      ]} />
    </>
  );
}
