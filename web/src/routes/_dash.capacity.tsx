...
// reading the Python-emitted data.capacity (no client derivation).
// L1: region · L2 (?workType=X): work-type capacity · L3 (?teamId=Y): team load
import { createFileRoute, Link, useNavigate } from "@tanstack/react-router";
import { useDash } from "@/dashboards/app-context";
// @ts-expect-error jsx
import { Breadcrumb, ThreadNav, PillRow, Panel, AurRiskBadge, ForecastPill, DriverList, AurChip, aurMono, aurSerif } from "@/dashboards/atoms.jsx";
// @ts-expect-error jsx
...
function Capacity() {
  ...
  if (teamId) return <TeamLevel cap={cap} teamId={teamId} AUR={AUR} />;
  if (workType) return <WorkTypeLevel data={data} cap={cap} workType={workType} AUR={AUR} />;
  ...
  return (
    <>
      ...
      <Panel AUR={AUR} pad={0} style={{ overflow: "hidden" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1.2fr 0.8fr 1fr 0.7fr 28px", padding: "16px 22px", borderBottom: `1px solid ${AUR.border}` }}>
          {["Work type","Utilization","SLA now","Forecast","Risk",""].map((h, i) => (
            <span key={i} style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, letterSpacing: 0.6, textTransform: "uppercase", textAlign: i === 2 ? "right" : "left" }}>{h}</span>
          ))}
        </div>
        {cap.byWorkType.map((w: any, i: number) => (
          <Link key={w.work_type} to="/capacity" search={((prev: any) => ({ ...prev, workType: w.work_type, teamId: undefined })) as any}
            style={{
              display: "grid", gridTemplateColumns: "1.6fr 1.2fr 0.8fr 1fr 0.7fr 28px",
            ...
            <span style={{ fontFamily: aurMono, fontSize: 12.5, color: statusColor(AUR, classifyMetric("sla", w.slaNow)), textAlign: "right" }}>{w.slaNow.toFixed(1)}</span>
...
// ─── L2 · Work-type ─────────────────────────────────────────────────────────
function WorkTypeLevel({ data, cap, workType, AUR }: any) {
  const { densityPreset } = useDash();
  const navigate = useNavigate();
  const w = cap.byWorkType.find((x: any) => x.work_type === workType);
  if (!w) return <div style={{ padding: 40, color: AUR.textFaint, textAlign: "center", fontStyle: "italic" }}>Work type not found.</div>;
  const teams = cap.teams.filter((t: any) => t.work_type === workType).sort((a: any, b: any) => b.riskScore - a.riskScore);

  const tiles = [
    { label: "SLA now",      value: `${w.slaNow.toFixed(1)}%`, status: classifyMetric("sla", w.slaNow) },
    ...
    { label: "Complexity",   value: `${w.complexity}%`, sub: "high-complexity", status: w.complexity > 50 ? "warn" : "good" },
  ];

  return (
    <>
      <Breadcrumb AUR={AUR} items={[
        { label: "Module D · Capacity & SLA", to: "/capacity", search: ((prev: any) => ({ ...prev, workType: undefined, teamId: undefined })) },
        { label: WORK_TYPE_LABELS[workType] },
      ]} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.accent, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 10 }}>Module D · {WORK_TYPE_LABELS[workType]}</div>
        ...
        <PillRow AUR={AUR} options={data.workTypeRollup.map((r: any) => r.work_type)} value={workType}
          onChange={(v: string) => navigate({ to: "/capacity", search: ((prev: any) => ({ ...prev, workType: v, teamId: undefined })) as any })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: densityPreset.sectionGap }}>
        <Panel AUR={AUR}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 18 }}>
            <span style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, letterSpacing: 0.6, textTransform: "uppercase" }}>Capacity risk drivers</span>
          ...
          <DriverList AUR={AUR} drivers={w.drivers} max={6} />
        </Panel>
        ...
        <Panel AUR={AUR} glow accentEdge style={{ display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "84px 1fr", gap: "11px 16px", fontSize: 13.5, marginTop: "auto" }}>
            <span style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, textTransform: "uppercase", letterSpacing: 0.5 }}>Owner</span><span style={{ color: AUR.text }}>Workforce planning lead + {WORK_TYPE_LABELS[workType]} managers</span>
            ...
            <span style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, textTransform: "uppercase", letterSpacing: 0.5 }}>Decision</span><span style={{ color: AUR.accent }}>{w.decision}</span>
          </div>
        </Panel>
      </div>

      <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.textFaint, letterSpacing: 0.6, textTransform: "uppercase", marginTop: densityPreset.sectionGap, marginBottom: 16 }}>Team load · {WORK_TYPE_LABELS[workType]}</div>
      <Panel AUR={AUR} pad={0} style={{ overflow: "hidden" }}>
        ...
        {teams.map((t: any, i: number) => (
          <Link key={t.team_id} to="/capacity" search={((prev: any) => ({ ...prev, workType, teamId: t.team_id })) as any}
            style={{
            ...
            <div>
              ...
              <div style={{ fontSize: 11.5, color: AUR.textFaint, marginTop: 2 }}>{t.name} · {t.contributors} heads{t.shift ? ` · ${t.shift}` : ""}</div>
            </div>
            <div style={{ paddingRight: 18 }}>
              <div style={{ height: 6, background: AUR.surfaceHi, borderRadius: 6, overflow: "hidden" }}>
      ...
      <ThreadNav AUR={AUR} density={densityPreset} items={[
        { kicker: "Module A", title: "Health for this work type", hint: `SLA, CSAT and escalations for ${WORK_TYPE_LABELS[workType]}.`, to: "/health", search: ((prev: any) => ({ ...prev, workType, teamId: undefined })) },
        { kicker: "Module C", title: "Is quality slipping under load", hint: "Quality risk often trails capacity strain.", to: "/workforce", search: ((prev: any) => ({ ...prev, workType, teamId: undefined })) },
        { kicker: "Module B", title: "Recurring escalations here", hint: "Capacity shortfalls show up as repeat patterns.", to: "/patterns", search: ((prev: any) => ({ ...prev, workType, teamId: undefined, patternId: undefined })) },
      ]} />
    </>
  );
}
...
// ─── L3 · Team ──────────────────────────────────────────────────────────────
function TeamLevel({ cap, teamId, AUR }: any) {
  const { densityPreset } = useDash();
  const t = cap.teams.find((x: any) => x.team_id === teamId);
  ...
  return (
    <>
      <Breadcrumb AUR={AUR} items={[
        { label: "Module D · Capacity & SLA", to: "/capacity", search: ((prev: any) => ({ ...prev, workType: undefined, teamId: undefined })) },
        { label: WORK_TYPE_LABELS[t.work_type], to: "/capacity", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: undefined })) },
        { label: t.team_id.replace("TEAM_APAC_", "") },
      ]} />
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24, flexWrap: "wrap" }}>
        <div>
          <div style={{ fontFamily: aurMono, fontSize: 10.5, color: AUR.accent, letterSpacing: 0.8, textTransform: "uppercase", marginBottom: 10 }}>{WORK_TYPE_LABELS[t.work_type]}{t.city ? ` · ${t.city}` : ""}{t.shift ? ` · ${t.shift} shift` : ""}</div>
          <h1 style={{ fontFamily: aurSerif, fontSize: 36, fontWeight: 400, letterSpacing: -1, margin: 0, lineHeight: 1.1, color: AUR.text }}>{t.name}'s team · capacity load</h1>
          <p style={{ color: AUR.textDim, fontSize: 14, marginTop: 12, maxWidth: 720, lineHeight: 1.6 }}>
      ...
      <Panel AUR={AUR} glow accentEdge style={{ marginTop: densityPreset.sectionGap }}>
        ...
        <div style={{ fontFamily: aurSerif, fontSize: 24, color: AUR.text, letterSpacing: -0.4, lineHeight: 1.3 }}>{t.action}.</div>
      </Panel>
      ...
      <ThreadNav AUR={AUR} density={densityPreset} items={[
        ...
        { kicker: "Module C", title: "This team's quality detail", hint: "Is load translating into quality drift?", to: "/workforce", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: t.team_id })) },
        { kicker: "Module D", title: `Back to ${WORK_TYPE_LABELS[t.work_type]}`, hint: "Return to the work-type outlook.", to: "/capacity", search: ((prev: any) => ({ ...prev, workType: t.work_type, teamId: undefined })) },
      ]} />
    </>
  );
}
