...
// (read from data.leadership — emitted by the Python pipeline), module gateways.
// Each alert threads into the relevant module's deep level via go(section, focus).
import { createFileRoute, Link } from "@tanstack/react-router";
import { useDash } from "@/dashboards/app-context";
import { useGo } from "@/dashboards/nav";
// @ts-expect-error jsx
import { AurKpi, AurSection, aurMono, aurSerif, aurSans, Panel } from "@/dashboards/atoms.jsx";
// @ts-expect-error jsx
import { fmt, classifyMetric } from "@/dashboards/data-utils.jsx";
...
function Home() {
  ...
  // Module C/D + leadership now come straight from the payload — no client derivation.
  const wf = d.workforce, cap = d.capacity, ld = d.leadership;
  ...
  const gateways = [
    ...
    { to: "/workforce", eyebrow: "Module C",   title: "Workforce Quality", stat: String(wf.region.highRiskTeams), statLabel: "teams at quality risk", status: wf.region.highRiskTeams > 2 ? "bad" : wf.region.highRiskTeams ? "warn" : "good", body: "Where quality is drifting, and who needs coaching, calibration or support." },
    { to: "/capacity",  eyebrow: "Module D",   title: "Capacity & SLA",    stat: String(cap.region.atRisk), statLabel: "work types at SLA risk", status: cap.region.atRisk > 2 ? "bad" : cap.region.atRisk ? "warn" : "good", body: "Next-week SLA forecast, staffing gaps, and where to rebalance." },
  ];

  const alertColor = (kind: string) => (kind === "pattern" ? AUR.bad : kind === "capacity" ? AUR.warn : AUR.accent);
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: densityPreset.gap }}>
        ...
        <AurKpi AUR={AUR} label="Backlog"       value={fmt.num(k.backlog)} sub={`${cap.region.agedShare}% aged >72h`} status={k.aged_backlog_72h > 800 ? "bad" : k.aged_backlog_72h > 250 ? "warn" : "good"} trendData={d.kpiTrends.backlog} />
      ...
      {/* Leadership attention — straight from data.leadership */}
      <AurSection AUR={AUR} density={densityPreset} eyebrow="Leadership attention" title="Three things that matter this week." lede={ld.headline}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {ld.alerts.length === 0 && <Panel AUR={AUR}><div style={{ color: AUR.textFaint, fontStyle: "italic", textAlign: "center", padding: 24 }}>No leadership alerts this period. The region is holding the line.</div></Panel>}
          {ld.alerts.map((a: any, i: number) => {
            const color = alertColor(a.kind);
            return (
              <Panel key={i} AUR={AUR} hoverable accentEdge={a.kind === "pattern"} onClick={() => go(a.target.section, a.target.focus)}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 20, alignItems: "center" }}>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <span style={{ width: 8, height: 8, borderRadius: "50%", background: color, boxShadow: `0 0 12px ${color}99` }} />
                      <span style={{ fontFamily: aurMono, fontSize: 10.5, color, letterSpacing: 0.7, textTransform: "uppercase" }}>{a.kind} signal</span>
                    </div>
